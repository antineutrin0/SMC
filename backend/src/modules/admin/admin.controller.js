...
const getDashboardStats = async (req, res) => {
  try {
    const [[{ totalPatients }]]      = await db.query("SELECT COUNT(*) AS totalPatients FROM MedicalCard");
    const [[{ totalEmployees }]]     = await db.query("SELECT COUNT(*) AS totalEmployees FROM Employee WHERE is_active = 1");
    const [[{ pendingApplications }]]= await db.query("SELECT COUNT(*) AS pendingApplications FROM MedicalCardApplication WHERE ApplicationStatus = 'Pending'");
    const [[{ todayVisits }]]        = await db.query("SELECT COUNT(*) AS todayVisits FROM outdoor_visit WHERE DATE(visit_date) = CURDATE()");

    return ok(res, { data: { totalPatients, totalEmployees, pendingApplications, todayVisits } });
  } catch (err) { serverError(res, err, "admin.getDashboardStats"); }
};
...
const getEmployees = async (req, res) => {
  try {
    const [rows] = await db.query(
      "SELECT employee_id, full_name, designation, specialization, license_no, contact_no, photo_url, is_active FROM Employee ORDER BY designation, full_name"
    );
    return ok(res, { data: rows });
  } catch (err) { serverError(res, err, "admin.getEmployees"); }
};
...
const getEmployeeById = async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      "SELECT employee_id, full_name, designation, specialization, license_no, contact_no, photo_url, is_active FROM Employee WHERE employee_id = ?",
      [employeeId]
    );
    ...
    if (emp.designation === "Doctor") {
      const [[r]] = await db.query("SELECT COUNT(*) AS total FROM outdoor_visit WHERE doctor_id = ?", [employeeId]);
      activity.push({ type: "Total Visits", count: r.total });
    } else if (emp.designation === "Driver") {
      const [[r]] = await db.query("SELECT COUNT(*) AS total FROM ambulance_log WHERE driver_id = ?", [employeeId]);
      activity.push({ type: "Total Trips", count: r.total });
    } else if (emp.designation === "Nurse") {
      const [[r]] = await db.query("SELECT COUNT(*) AS total FROM medicine_dispensation WHERE dispensed_by = ?", [employeeId]);
      activity.push({ type: "Total Dispensations", count: r.total });
    }

    return ok(res, { data: { ...emp, activity } });
  } catch (err) { serverError(res, err, "admin.getEmployeeById"); }
};
...
const getApplications = async (req, res) => {
  try {
    ...
    return ok(res, { data: rows });
  } catch (err) { serverError(res, err, "admin.getApplications"); }
};
...
const reviewApplication = async (req, res) => {
  ...
  } catch (err) { serverError(res, err, "admin.reviewApplication"); }
};

const approveMedicalCard = async (req, res) => {
  try {
    const { applicationId, personId, cardId, height, weight, bloodGroup, password } = req.body;
    const approvedBy = req.user.id;

    if (!cardId || !personId || !password) return badRequest(res, "cardId, personId and password are required");
    ...
    await db.query(
      `INSERT INTO MedicalCard (CardID, IssueDate, ExpiryDate, Status, PersonID, Height_cm, Weight_kg, BloodGroup, PasswordHash)
       VALUES (?, ?, ?, 'Active', ?, ?, ?, ?, ?)`,
      [cardId, issueDate, expiryDate, personId, height || null, weight || null, bloodGroup || null, passwordHash]
    );

    return created(res, { data: { cardId } }, "Medical card created");
  } catch (err) { serverError(res, err, "admin.approveMedicalCard"); }
};
...
const getRosters = async (req, res) => {
  try {
    ...
    return ok(res, { data: rows });
  } catch (err) { serverError(res, err, "admin.getRosters"); }
};

const createRoster = async (req, res) => {
  try {
    const { employeeId, dutyType, startDate, endDate, shiftStart, shiftEnd } = req.body;
    ...
    const [result] = await db.query(
      `INSERT INTO roster (employee_id, duty_type, start_date, end_date, shift_start, shift_end, created_at, created_by, status)
       VALUES (?, ?, ?, ?, ?, ?, NOW(), ?, 'Draft')`,
      [employeeId, dutyType, startDate, endDate, shiftStart || null, shiftEnd || null, createdBy]
    );

    return created(res, { data: { rosterId: result.insertId } }, "Roster created");
  } catch (err) { serverError(res, err, "admin.createRoster"); }
};
...
const approveRoster = async (req, res) => {
  ...
  } catch (err) { serverError(res, err, "admin.approveRoster"); }
};
...
const updateRoster = async (req, res) => {
  ...
  } catch (err) { serverError(res, err, "admin.updateRoster"); }
};

const deleteRoster = async (req, res) => {
  try {
    const { rosterId } = req.params;
    await db.query("DELETE FROM roster WHERE roster_id = ? AND status = 'Draft'", [rosterId]);
    return ok(res, {}, "Roster deleted");
  } catch (err) { serverError(res, err, "admin.deleteRoster"); }
};
...
const getPatients = async (req, res) => {
  try {
    ...
    return ok(res, { data: rows });
  } catch (err) { serverError(res, err, "admin.getPatients"); }
};
...
const approveFirstAidRequest = async (req, res) => {
  try {
    ...
    if (status === "APPROVED") {
      const [items] = await db.query("SELECT * FROM first_aid_item WHERE request_id = ?", [requestId]);
      for (const item of items) {
        ...
        await db.query(
          ...
          [item.medicine_id, item.quantity, approvedBy, requestId, total - item.quantity]
        );
      }
    }

    return ok(res, {}, `First aid request ${status.toLowerCase()}`);
  } catch (err) { serverError(res, err, "admin.approveFirstAidRequest"); }
};

module.exports = {
  getDashboardStats, getEmployees, getEmployeeById,
  getApplications, reviewApplication, approveMedicalCard,
  getRosters, createRoster, approveRoster, updateRoster, deleteRoster,
  getPatients, approveFirstAidRequest,
};
